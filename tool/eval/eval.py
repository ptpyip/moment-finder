...
def compute_mr_rk(submission, ground_truth, iou_thds=np.linspace(0.5, 0.95, 10), k=5):
    """Custom code: get R@k metric"""
    # print(submission[0])
    if k == 1:
    ...
    for d in ground_truth:
        ...
        cur_max_iou_idx = (0, 0)
        if len(cur_gt_windows) > 0:  # select the GT window that has the highest IoU
            # print(pred_qid2windows)
            cur_ious = compute_temporal_iou_batch_cross(
                np.array(pred_qid2windows[cur_qid]), np.array(d["relevant_windows"])
            )[0]
            cur_max_iou_idx = np.unravel_index(np.argmax(cur_ious, axis=None), cur_ious.shape)
        pred_qid2window[cur_qid] = pred_qid2windows[cur_qid][cur_max_iou_idx[0]]
        gt_qid2window[cur_qid] = cur_gt_windows[cur_max_iou_idx[1]]
...
def eval_moment_retrieval(submission, ground_truth, verbose=True):
    ...
    for l_range, name in zip(length_ranges, range_names):
        ...
        iou_thd2recall_at_five = compute_mr_rk(_submission, _ground_truth)
        ret_metrics[name] = {
